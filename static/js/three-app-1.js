(function main() {
	...
	NeuralNetwork.prototype.initNeurons = function(inputVertices) {
		...
		for (var i = 0; i < inputVertices.length; i += this.verticesSkipStep) {
			...
			if (n.type == EXCITOR){
				...
				//exciteNeurons.push(this.allNeurons.length-1);
			}
			else if (n.type == INHIBITOR) {
				this.inhibitorsGeom.vertices.push(n);
				//inhibNeurons.push(this.allNeurons.length-1);
			}
		}
	...
	NeuralNetwork.prototype.initAxons = function () {
			...
			for (var j=0; j<allNeuronsLength; j++) {
				...
				/*var constring = "";
				var wstring = "";*/
				for (var k=0; k<allNeuronsLength; k++) {// changed from j+1 to 0
						...
						n2.connection.length < this.maxConnectionPerNeuron)
					{
						...
						if(rand === 0){
							...
							//constring = constring.concat("1");
							//wstring = wstring.concat(connectedAxon.weight.toString());
							//this.connections[k]][j] = 1;
							//this.connWeight[j][k] = connectedAxon.weight;
							//this.connWeight[k][j] = connectedAxon.weight;
						}
						...
						//constring = constring.concat("0");
						//wstring = wstring.concat("0");
					}
					if(k < allNeuronsLength-1){
						//constring = constring.concat(",");
			...
			//for (i=0; i < this.connections.length; i++){
				/*var constring = this.connections[i];
				var wstring = this.connWeight[i];
				$.ajax({
				 	type: "POST",
				 	url: "/connection",
				  	contentType: "application/json; charset=utf-8",
				  	data: JSON.stringify(constring),
				  	dataType: 'json',
				  	success: function(data) {
				  		console.log("success");
				  	},
